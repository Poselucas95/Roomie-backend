...
async function getMatches(userId, prop) {
    ...
    try{
        ...
        // Ejecución de la query
        if(prop === 'true'){
            query= "SELECT p.Apodo, fp.Value as Foto FROM Propiedad pr LEFT JOIN Matchs m ON pr.IdPropiedad = m.IdPropiedad LEFT JOIN Perfil p ON p.IdFirebase = m.IdFirebase OUTER APPLY (SELECT TOP 1 f.* FROM  FotoPerfil f WHERE  f.IdFirebase = m.IdFirebase) fp WHERE m.Estado = 'Match' AND pr.IdFirebase = @IdFirebase"
        }
        if(prop === 'false'){
